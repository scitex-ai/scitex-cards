...
_BACKEND_CLAIM = re.compile(
    r"\b(the retired engine|PostgreSQL|Postgres|MySQL|DuckDB)\s+(database|store|task store|db)\b",
    re.IGNORECASE,
)
...
_BACKEND_LITERAL = re.compile(r"backend:\s*(the retired engine|postgres|postgresql|mysql)\b", re.IGNORECASE)
...
def test_the_detector_actually_fires():
    ...
    planted = (
        "A canonical the retired engine task store with pluggable adapters.\n"
        "# -> prints {resolved: <path>, backend: the retired engine, ...}\n"
    )
    # Act
...
def test_the_detector_does_not_fire_on_honest_history():
    ...
    honest = (
        "That zero-config tier was deleted 2026-08-13 (operator ruling: the retired engine "
