...
@pytest.fixture
def importable():
    """Put a directory on the REAL ``sys.path``; take it off on teardown.

    Replaces `monkeypatch.syspath_prepend`. This writes the same
    ``sys.path`` the import system reads — the package genuinely becomes
    importable — and restores the list afterwards, which is the only thing
    monkeypatch was providing. Nothing is faked: these fixtures build real
    packages on disk and this makes them findable, which is the whole point
    of a probe that reports on installs.
    """
    added = []
    ...
    for entry in added:
        ...
        except ValueError:  # someone else already tidied it
            pass


@pytest.fixture
def fake_pkg(tmp_path, importable):
...
@pytest.fixture
def wheel_pkg_with_two_distinfos(tmp_path, importable):
...
# --------------------------------------------------------------------------
def _claims(version: str):
    """A metadata lookup that CLAIMS ``version``, to pass as ``version_of``.

    Replaces `monkeypatch.setattr("..._install_probe._md.version", ...)`. The
    probe exists to catch metadata DISAGREEING with the code on disk, so the
    metadata side has to be controllable — but rebinding the attribute edits
    the module under test in order to test it. `probe_install(version_of=...)`
    takes the lookup, so the claim is stated at the call.
    """
    return lambda _dist: version


@pytest.fixture
def fossilised_probe(fake_pkg):
    """Metadata says 1.0.0; the code on disk is 2.0.0."""
    return probe_install("fakepkg", version_of=_claims("1.0.0"))


@pytest.fixture
def agreeing_probe(fake_pkg):
    """Metadata and source agree at 2.0.0 — the healthy shape."""
    return probe_install("fakepkg", version_of=_claims("2.0.0"))


@pytest.fixture
def orphaned_probe():
    """Metadata present, code absent — every version check "passes" on nothing."""
    return probe_install("ghostpkg", version_of=_claims("0.7.26"))
...
def exploding_metadata_probe(fake_pkg):
    ...
    return probe_install("fakepkg", version_of=boom)  # must not raise
...
def health_check_result(fake_pkg):
    ...
    return check_install_honest("fakepkg", version_of=_claims("1.0.0"))
...
def ambiguous_probe(wheel_pkg_with_two_distinfos):
    ...
    return probe_install("wheelpkg", version_of=_claims("0.7.50"))


@pytest.fixture
def solo_distinfo_probe(tmp_path, importable):
    ...
    importable(site)
    yield probe_install("solopkg", version_of=_claims("1.0.0"))
