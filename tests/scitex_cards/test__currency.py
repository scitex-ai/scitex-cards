...
``check_currency(staleness, BARE_HOST)`` delegates to ``scitex_dev.staleness.ensure_current`` when
...
#: The gate's "actor CAN remediate" fact, passed to `check_currency` rather
#: than arranged by rebinding `_running_over_overlay`. This interpreter's
#: site-packages really is on an overlay, so the bare-host branch is otherwise
#: unreachable here except by rewriting the module under test.
BARE_HOST = False


class _AbsentStaleness:
    """A staleness module that behaves like scitex-dev not being installed.

    Every attribute access raises ``ImportError``, which is exactly what the
    import machinery hands `check_currency` / `currency_verdict` when the
    module is missing — so the code under test meets the same exception at the
    same boundary. A hand-rolled fake, which is what the rule asks for when the
    real collaborator's ABSENCE is the condition being tested; scitex-dev is
    installed here, so absence cannot be arranged any other way in-process.
    """

    def __getattr__(self, name):
        raise ImportError("No module named 'scitex_dev.staleness'")


def _staleness_module(ensure_current, stale_error=None):
    """A hand-rolled `scitex_dev.staleness` to PASS IN, not to install.

    Replaces `monkeypatch.setitem(sys.modules, ...)`. `check_currency` and
    `currency_verdict` take the checker as a parameter, so the test states
    which checker it means instead of editing the interpreter's module table
    to fake an import that would really have succeeded.

    `stale_error`, when given, is published as the module's `StalenessError`
    so `currency_verdict(staleness)` can tell a real staleness VERDICT apart from
    scitex-dev malfunctioning (the latter is `unknown`, not `stale`).
    """
    module = types.ModuleType("scitex_dev.staleness")
    module.ensure_current = ensure_current
    if stale_error is not None:
        module.StalenessError = stale_error
    return module


def _stale_fake(message="scitex-cards 0.17.7 is behind latest 0.17.9"):
    """The stale-install world: a fresh verdict cache + a refusing checker.

    Returns ``(staleness_module, message)``. The cache reset now goes through
    the public `reset_currency_state()` instead of `monkeypatch.setattr` on two
    private globals — same guarantee (no leak between tests), without the suite
    depending on the names of internals it does not own.
    """
    ...
    return _staleness_module(_fake_ensure_current, _FakeStalenessError), message
...
def test_check_currency_no_ops_when_scitex_dev_lacks_the_staleness_module():
    # Arrange — a checker that behaves like scitex-dev not being installed,
    # regardless of whether it happens to be installed in this environment.
    staleness = _AbsentStaleness()

    # Act
    outcome = check_currency(staleness, BARE_HOST)

    # Assert — returns without raising; scitex-cards stays standalone. The
    # RETURN is the evidence: this used to be an assertion-free test, which
    # proved only that the line executed.
...
def test_check_currency_passes_through_when_the_install_is_current():
    ...
    calls = []
    staleness = _staleness_module(calls.append)

    # Act
    check_currency(staleness, BARE_HOST)
...
def test_check_currency_raises_when_the_install_is_stale():
    ...
    staleness = _staleness_module(_fake_ensure_current)
...
# --------------------------------------------------------------------------- #
# (d) currency_verdict(staleness) — the NON-RAISING sibling, three-valued              #
#                                                                             #
...
def test_currency_verdict_reports_unknown_when_scitex_dev_is_absent():
    ...
    # Act
    verdict = currency_verdict(staleness)
...
def test_currency_verdict_reports_current_when_the_install_is_fresh():
    # Arrange
    staleness = _staleness_module(lambda dist_name: None)
...
def test_currency_verdict_reports_stale_when_scitex_dev_refuses():
    # Arrange
    staleness, _message = _stale_fake()
...
def test_currency_verdict_is_unknown_when_scitex_dev_itself_malfunctions():
    ...
    staleness = _staleness_module(_fake_ensure_current, _FakeStalenessError)
...
# --------------------------------------------------------------------------- #
# (e) warn_if_stale_once(staleness) — the Python rail's notice                        #
# --------------------------------------------------------------------------- #
def test_warn_if_stale_once_does_not_raise_when_the_install_is_stale():
    ...
    # Act
    verdict = warn_if_stale_once(staleness)

    # Assert
    assert verdict.state == "stale"


def test_warn_if_stale_once_emits_a_warning_when_the_install_is_stale(caplog):
    # Arrange
    staleness, _message = _stale_fake()
    caplog.set_level(logging.WARNING, logger=_CURRENCY_LOGGER)

    # Act
    warn_if_stale_once(staleness)

    # Assert
    assert len(_currency_warnings(caplog)) == 1


def test_warn_if_stale_once_warns_exactly_once_across_repeated_calls(caplog):
...
def test_warn_if_stale_once_warning_quotes_scitex_devs_message_verbatim(caplog):
    # Arrange
    staleness, message = _stale_fake()
    caplog.set_level(logging.WARNING, logger=_CURRENCY_LOGGER)
...
def _raising_fake(exc):
    ...
    return _staleness_module(_fake_ensure_current, _FakeStalenessError)
...
def test_warn_if_stale_once_swallows_a_system_exit_from_the_currency_path():
    ...
    # Arrange
    staleness = _raising_fake(SystemExit("scitex-dev called sys.exit()"))
...
def test_warn_if_stale_once_lets_a_keyboard_interrupt_propagate():
    ...
    # Arrange
    staleness = _raising_fake(KeyboardInterrupt())
...
def test_currency_verdict_keeps_the_stale_verdict_when_the_message_cannot_render():
    ...
    staleness = _staleness_module(_fake_ensure_current, _UnrenderableStalenessError)
...
def test_currency_verdict_is_unknown_when_staleness_error_is_not_an_exception():
    ...
    staleness = _staleness_module(_fake_ensure_current, "not an exception class")
