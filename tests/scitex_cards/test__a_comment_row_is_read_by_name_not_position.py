...
def _positional_reads_of_fetched_rows(source: Path) -> list[tuple[str, int]]:
    ...
    for every opt-in caller until the one-card write's PostgreSQL test hit it
    on 2026-09-02. So this guard keys on WHERE THE VALUE CAME FROM, not what it
    was called: a name assigned from a fetch, or iterated from one, must never
    be subscripted with an integer, in any module of the package.
    """
    tree = ast.parse(source.read_text(encoding="utf-8"))
    fetched: set[str] = set()
    for node in ast.walk(tree):
