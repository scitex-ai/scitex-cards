...
``migration plan`` is the read-side dry-run scanner; ``migration apply`` is
the operator-blessed mutation. The group token is a NOUN (``migration``)
per the SciTeX noun-verb CLI convention (audit-cli §1, non-leaf nodes are
nouns).

Named ``_migration_cli.py`` (not ``_migration.py``) to avoid colliding
with the existing ``scitex_todo/_migration/`` engine package this module
calls into.
...
@click.group(
    ...
    help_spec=CliHelp(
        ...
        examples=(
            Example("{prog} migration plan --json", "Preview the plan as JSON."),
            Example("{prog} migration apply -y", "Run the migration."),
        ),
    ),
)
...
@migration_group.command(
    ...
    help_spec=CliHelp(
        ...
        examples=(
            Example("{prog} migration plan --json", "Machine-readable plan."),
            Example("{prog} migration plan --markdown", "Operator-facing review doc."),
        ),
    ),
)
@click.option(
...
@migration_group.command(
    ...
    help_spec=CliHelp(
        ...
        examples=(
            Example("{prog} migration apply --dry-run", "Preview without writing."),
            Example("{prog} migration apply -y", "Run the migration."),
        ),
    ),
)
...
def register(main: click.Group) -> None:
    """Attach the ``migration`` noun group to the root CLI."""
