...
@click.command(
    ...
    help_spec=CliHelp(
        ...
        examples=(
            Example("{prog} sync-github --dry-run", "Preview the planned actions."),
            Example("{prog} sync-github --since 2026-06-01 -y", "Sync since a date."),
            Example("{prog} sync-github -y", "Cron / scripted use."),
        ),
    ),
)
@click.option(
...
def sync_github_cmd(
    ...
    The aggregation function is shared with ``print-stats`` and the WIP
    gate via :mod:`scitex_todo._throughput`.
