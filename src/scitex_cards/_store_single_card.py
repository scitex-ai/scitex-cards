...
from ._store_canonical_read import _guarded_connection, _missing_table_errors
from ._store_errors import RevisionConflictError, StoreNotProvisionedError
...
def read_card_or_raise(target: str, task_id: str) -> tuple[dict | None, int | None]:
    ...
    raises on: unreachable server, stamped for another store, retired store,
    no ``tasks`` table, a row with no payload. A one-card read that answered
    ...
    try:
        try:
            ...
            # The PostgreSQL guard asks the catalogue before this SELECT; the
            # SQLite guard deliberately does not (its whole-document twin lets
            # ``open_db`` create the schema), so a schemaless file reaches here.
            # SAME VERDICT as the server branch: not provisioned, not empty.
            raise StoreNotProvisionedError(
                f"the store {target!r} has no `tasks` table. REFUSING to read "
                f"card {task_id!r} from it: a schemaless store is not an empty "
                f"board, and answering 'not found' here would say the card is "
                f"gone when it is the store that is missing."
            ) from exc
        if row is None:
            return None, None
...
def write_card_or_raise(
    ...
    try:
        if not _is_postgres(conn):
            # Take the SQLite write lock at the START, so the compare-and-set's
            # read and its write happen under one lock rather than racing
            # between them. psycopg opens its transaction on the first
            # statement and the ``WHERE tasks.revision = ?`` is atomic there.
