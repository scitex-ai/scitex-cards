    ...
    *,
    ...
    if isinstance(exc, AgentIdentityUnresolved):
        ...
        if "token" in detail or "auth" in detail or "bearer" in detail:
            ...
            action = (
                "run `scitex-cards hub doctor`, re-provision the token, then retry"
            )
    ...
    return StatusCode(
        ...
        code=code,
        message=(
            f"OBSERVED: {observation}. {persistence} NEXT: {action}."
        ),
    )
    ...
    try:
        with ledger:
            ...
            if record.get("idempotent_replay"):
                ...
                notification = notification_for_exchange(
                    original_exchange, store=store
                )
