...
MEASURED 2026-07-24, not hypothetical. The scitex-cards -> scitex-cards rename
re-registered this MCP server as ``scitex-cards`` while agent launch lines still
allowlisted the pre-rename ``scitex-cards``. The whole fleet went deaf to the
...
OUR_CLI_NAMES = frozenset({"scitex-cards", "scitex-cards"})
...
def _runs_our_cli(spec: dict[str, Any]) -> bool:
    ...
    for index, token in enumerate(tokens):
        ...
        if previous == "-m":
            if token.startswith(("scitex_cards", "scitex_cards")):
                return True
            continue
