...
# --------------------------------------------------------------------------- #
#
# STORE checks — MOVED to `_health_store` (this file hit the 512 cap), on the
# same terms as the `_health_cards` split below: the import surface does NOT
# move, and every name is the SAME object it always was, defined next door.
from ._health_store import (  # noqa: E402,F401  (re-export)
...
def _check_delivery_liveness(store: str | Path | None) -> dict[str, Any]:
    """Is the daemon's TICKING actually DELIVERING? (``notifyd_alive`` is not this.)

    ``notifyd_alive`` answers "is the process there and stamping a heartbeat?"
    On 2026-07-28 the answer was yes for a whole day while EVERY tick failed to
    read the store: the heartbeat was fresh, the summary line said
    ``sent=0 failed=0``, and the operator's DMs went nowhere. Liveness of the
    process is not liveness of the delivery. This check reads the record the
    loop persists each tick (:mod:`._delivery._liveness`) and reports the
    consecutive-failure count, the underlying reason, and when delivery last
    actually worked.
    """
    from ._delivery._liveness import assess_delivery_liveness

    return assess_delivery_liveness(store)
...
def health(
    *,
    ...
    checks = [
        ...
        # Is the daemon's ticking DELIVERING anything? `notifyd_alive` was GREEN
        # through a day-long outage in which every tick failed its store read —
        # a fresh heartbeat proves the process is there, not that it works.
