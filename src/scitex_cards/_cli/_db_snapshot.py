#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""The ``scitex-cards db snapshot`` verb — the ADR-0010 off-site backup rail.

Its own module because ``_db.py`` was at 533 lines against the 512-line cap
when the card ``hm01-audit-burndown-2026-08-16`` audit edits were landing.
The command is attached by :func:`register`, which takes the group as an
argument rather than importing it, so there is no import cycle — the same
pattern as ``_board_service.py``.

Every guard this verb carries (shrink refusal, the two freshness asserts, the
previous-count parser) lives HERE because only the snapshot uses them;
``_db.py`` re-exports the names so the test suite's existing
...
def register(db_group: click.Group) -> None:
    """Attach ``snapshot`` to the ``db`` noun group."""
    db_group.add_command(db_snapshot_cmd)
...
def db_snapshot_cmd(
    ...
    if as_json:
        ...
        return
    # The shared report printer stays with the `db` group module; import it
    # lazily here, like every other heavy import in this body.
    from ._db import _echo_export_report
