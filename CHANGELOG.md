...
`_guarded_connection`, the ONE definition of exists / ownership / retired for
both backends; the whole-document read calls it and so does the one-card path.
There is no lenient single-card variant, because the 2026-07-19 outage was the
read door and the write door disagreeing. The export's `COUNT(*)` cross-check
stays exactly where it is and exactly as strict for every caller that still
produces a whole document (`list_tasks`, `db export`, backup, the board); the
one-card verbs simply no longer produce one.
...
Tests: `test__store_single_card.py` (SQLite tmp stores; the guard refusals as
positive controls; cProfile asserting `export_doc` ran 0 times) and
`test__single_card_write_on_postgres.py` (a throwaway schema on a real
PostgreSQL, discovered by the postgres-backend job).

### The compare-and-set upsert died on PostgreSQL, and the SQLite suite could not see it

Found by that PostgreSQL test on its first run. `_db_bootstrap._insert_tasks`
read the current revision with `found_row[0]` (and the post-race re-read with
`after[0]`), which `sqlite3.Row` accepts and psycopg's `dict_row` answers with
`KeyError: 0`. So every caller that opted into `expected_revision` —
`update_task(expected_revision=…)`, the incremental mirror's compare-and-set,
and now the one-card write — crashed on the engine the fleet runs, while every
SQLite test passed. Same class as the 2026-08-23 comment-merge outage, one
module over. Both reads are by name now.

The source guard that was supposed to catch this keyed on the identifier
`row` in one module. It now keys on WHERE THE VALUE CAME FROM: any name bound
from `.fetchone()` / `.fetchall()`, in any module of the package, subscripted
with an integer, fails
`test__a_comment_row_is_read_by_name_not_position.py`. The three other sites it
found (`_db_foreign_keys`, `_health_stranded_backlog`, `_inbox_migrate_postgres`)
were on SQLite-only paths and are converted to the shape-agnostic helpers the
package already had, so the guard holds with no allowlist.
